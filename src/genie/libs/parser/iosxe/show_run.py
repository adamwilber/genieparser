...
class ShowRunAllSchema(MetaParser):
...
# ==================================================
class ShowRunAll(ShowRunInterfaceSchema):
    ...
    def cli(interface=None, output=None):
        ...
        # radius server RADIUS_1
        p113 = re.compile(r'^(?P<server_type>\S+)\sserver\s(?P<server_name>\S+)$')

        # address ipv4 11.15.24.213 auth-port 1812 acct-port 1813
        p114_1 = re.compile(r'^address\s(?P<address_type>\S+)\s(?P<address>\S+)\sauth-port\s(?P<auth_port>\S+)\sacct-port\s(?P<acct_port>\S+)$')

        # address ipv4 11.15.24.213
        p114_2 = re.compile(r'^address\s(?P<address_type>\S+)\s(?P<address>\S+)$')

        # key Cisco123
        p115 = re.compile(r'^key\s(?P<key>\S+)$')

        # aaa group server radius RADIUS_GROUP
        p116 = re.compile(r'^aaa\sgroup\sserver\s(?P<server_type>\S+)\s(?P<server>\S+)$')

        # server name RADIUS_1
        p117 = re.compile(r'^server\sname\s(?P<server_name>\S+)$')

        # ip vrf forwarding newVRF2
        p118 = re.compile(r'^ip\svrf\sforwarding\s(?P<vrf>\S+)$')

        # ip radius source-interface TenGigabitEthernet1/0/13
        p119 = re.compile(r'^ip\s(?P<server_type>\S+)\ssource-interface\s(?P<source_interface>\S+)$')

        # aaa new-model
        p120 = re.compile(r'^aaa\s(?P<new_model>\S+)$')

        # aaa session-id common
        p121 = re.compile(r'^aaa\ssession-id\s(?P<session_id>\S+)$')

        # vrf definition ce1
        p122 = re.compile(r'^vrf +definition +(?P<vrf>\S+)$')

        #  rd 2:2
        p123 = re.compile(r'^rd +(?P<rd>[\d:]+)$')

        #   address-family ipv4
        #   address-family ipv4 unicast
        p124 = re.compile(r'^address\-family +(?P<af>ipv4|ipv6|ipv4 unicast|ipv4 multicast|ipv6 unicast|ipv6 multicast)$')

        # route-target import 3:201
        # route-target export 1:201
        # route-target both 65000:100
        # route-target export 100:1 stitching
        p125 = re.compile(r'^route\-target +(?P<type>import|export|both) +(?P<rt>[\d:]+)(\s+(?P<stitch>stitching))?$')

        # service <name>
        p126_1 = re.compile(r'^service\s(?P<name>\S+)$')
        # no service <name>
        p126_2 = re.compile(r'^no +service\s(?P<name>\S+)$')
        ...
        for line in output.splitlines():
            ...
            if m:
                group = m.groupdict()
                server_type_dict = config_dict.setdefault(group['server_type'],{})
                server_dict = server_type_dict.setdefault('server',{})
                server_name_dict = server_dict.setdefault(group['server_name'],{})
                continue
            ...
            if m:
                group = m.groupdict()
                group_server_dict = config_dict.setdefault('group_server',{})
                group_server_name_dict = group_server_dict.setdefault(group['server_type'],{})
                group_name_dict = group_server_name_dict.setdefault(group['server'], {})
                continue
            ...
            if m:
                vrf = m.groupdict()['vrf']
                vrf_dict = config_dict.setdefault('vrf', {}).setdefault(vrf, {})
                continue
            ...
            m = p126_1.match(line)
            if m:
                ser_name = m.groupdict()['name']
                service_dict = config_dict.setdefault('service', {}).setdefault(ser_name, True)

            # no service <name>
            m = p126_2.match(line)
            if m:
                ser_name = m.groupdict()['name']
                service_dict = config_dict.setdefault('service', {}).setdefault(ser_name, False)
